...
・管理者専用ページには以下のurlを入力すると画面遷移します。(http://localhost/admin/login)

≪以下11月18日追記≫

### csvインポートに関する注釈

・管理者は管理画面からcsvインポートにより店舗の新規登録が可能

![スクリーンショット 2024-11-19 094054](https://github.com/user-attachments/assets/f12f3c37-1e30-4a0d-9539-c2c16dc30a12)

・Googleスプレッドシートを開き1列目に「shop_name」、「area_id」、「genre_id」、「content」を設定。それぞれshopsテーブルのカラムに対応しています。

・2列目に以下の記述に従ってそれぞれの内容に沿って入力を行ってください。

・「shop_name」には店舗名を文字列で50字以内で記述。

・「area_id」にはareasテーブルに登録されているIDの数値で記述。1＝東京都、2＝大阪府、3＝福岡県に該当します。

・「genre_id」にはgenresテーブルに登録されているIDの数値を記述。1＝寿司、2＝焼肉、3＝居酒屋、4＝イタリアン、5＝ラーメンに該当します。

・「content」には店舗の概要を文字列で400字以内で記述。

・上記の項目に一つでも誤りがあるとインポート時にエラーが表示されます。
...
・登録画面から店舗画像と店舗情報の入ったcsvファイルをセットして登録してください。両方のファイルが無い状態では登録できません。

・店舗画像はjpeg、もしくはpng形式の物でないとエラー、csvファイルは拡張子がcsv形式の物でないとエラーが表示されます。
